    ...
    <div class="rounded-3xl bg-white/5 p-8 ring-1 ring-white/10">
      <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p class="mt-2 text-sm text-white/70">Depósitos e transferências de todos os clientes (Ledger).</p>
        </div>
      ...
      <!-- Filtros -->
      <div class="mt-6 grid gap-3 rounded-2xl bg-black/20 p-5 ring-1 ring-white/10 sm:grid-cols-6">
        ...
        <div>
          <div class="text-xs font-semibold text-white/70">Tipo</div>
          <select
        ...
        <div class="sm:col-span-6 flex flex-wrap justify-end gap-3">
          <button
            type="button"
      ...
      <div class="mt-6 overflow-hidden rounded-2xl ring-1 ring-white/10">
        <div class="grid grid-cols-12 gap-2 bg-black/20 px-5 py-3 text-xs font-semibold text-white/75">
          ...
          <div class="col-span-2">De</div>
          <div class="col-span-2">Para</div>
          <div class="col-span-1 text-center">Status</div>
          <div class="col-span-1 text-right">Valor</div>
        </div>

        <div v-if="loading" class="px-5 py-6 text-sm text-white/70">Carregando...</div>
        <div v-else-if="rows.length === 0" class="px-5 py-6 text-sm text-white/70">Nenhuma transação encontrada.</div>

        <div v-else class="divide-y divide-white/10">
          <div v-for="r in rows" :key="r.id" class="grid grid-cols-12 items-center gap-2 px-5 py-4 text-sm">
            <div class="col-span-4 text-white/80">{{ formatDate(r.created_at) }}</div>
            <div class="col-span-2 font-semibold text-white/85">{{ r.type }}</div>
            <div class="col-span-2 text-white/80">
              {{ r.from_user_email || 'Plataforma' }}
            </div>
            <div class="col-span-2 text-white/80">
              {{ r.to_user_email || '—' }}
            </div>
            ...
            <div class="col-span-1 text-right font-semibold text-[#e9c15e]">{{ formatBRL(r.amount) }}</div>
          </div>
        </div>
      </div>
...
async function fetchRows() {
  ...
  try {
    ...
    error.value = e?.response?.data?.message || 'Não foi possível carregar as transações.'
  } finally {
    loading.value = false
  }
}
